...
const LayerData &Model::infrence(const LayerData &inData,
                                 const Layer::InfType infType) const {
  assert(layers.size() > 0 &&
         "There must be at least 1 layer to perform infrence");
...
// Run infrence on a single layer of the model using the inData and outputting
// the outData infType can be used to determine the infrence function to call
const LayerData &Model::infrenceLayer(const LayerData &inData,
                                      const int layerNum,
                                      const Layer::InfType infType) const {
  ...
  assert(layer.isOutputBufferAlloced() &&
         "Output buffer must be allocated prior to infrence");
