...
public:
  ...
  // Functions
  const LayerData &
  infrence(const LayerData &inData,
           const Layer::InfType infType = Layer::InfType::NAIVE) const;
  const LayerData &
  infrenceLayer(const LayerData &inData, const int layerNum,
                const Layer::InfType infType = Layer::InfType::NAIVE) const;
  ...
  void insertLayer(Layer *l, std::size_t idx) {
    layers.insert(layers.begin() + idx, l);
  }

  // Remove a layer from the model
  void removeLayer(const std::size_t idx) {
  ...
  const LayerData &
  operator()(const LayerData &inData,
             const Layer::InfType infType = Layer::InfType::NAIVE) const;
  const LayerData &
  operator()(const LayerData &inData, const int layerNum,
             const Layer::InfType infType = Layer::InfType::NAIVE) const;
