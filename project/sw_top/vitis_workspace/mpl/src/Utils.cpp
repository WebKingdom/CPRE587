...
// Define out parser
static error_t parse_opt(int arg_key, char *arg_val,
                         struct argp_state *arg_state);
...
static error_t parse_opt(int arg_key, char *arg_val,
  ...
  case ARGP_KEY_ARG:
    ...
    break;
  case ARGP_KEY_END:
    if (args->singleLayer && args->layerNum < 0)
      argp_failure(arg_state, 1, 0, "You must specify a layer number to run");
    // else if (args->basePath.empty()) argp_failure(arg_state, 1, 0, "No input
    // data base path given");
    break;
