...
class LayerParams {
public:
  LayerParams(const std::size_t elementSize,
...
class LayerData {
public:
  LayerData(const LayerParams &params)
      : params(params), alloced(false), data(nullptr) {}
  ...
  bool compareWithin(const LayerData &other,
                     const T_EP epsilon = Config::EPSILON) const;

  // Compare within an Epsilon to ensure layer datas are similar within reason
  template <typename T, typename T_EP = float>
  bool compareWithinPrint(const LayerData &other,
                          const T_EP epsilon = Config::EPSILON) const;
...
public:
  // Contructors
  Layer(const LayerParams inParams, const LayerParams outParams,
        LayerType lType)
      : inParams(inParams), outParams(outParams), outData(outParams),
        lType(lType) {}
  virtual ~Layer() {}
...
template <typename T> void LayerData::allocData() {
  if (!alloced) {
    ...
    assert(false && "Cannot allocate a data pointer that has not been "
                    "allocated (LayerData)");
  }
}
...
template <typename T> inline void LayerData::loadData() {
  ...
  assert(!params.filePath.empty() &&
         "No file path given for required layer data to load from");
...
template <typename T> void LayerData::freeData() {
  if (alloced) {
    ...
    assert(false && "Cannot deallocate a data pointer that has not been "
                    "allocated (LayerData)");
  }
}
...
template <typename T> float LayerData::compare(const LayerData &other) const {
  ...
  if (aParams.elementSize != bParams.elementSize) {
    std::cerr << "Comparison between two LayerData arrays with different "
                 "element size (and possibly data types) is not advised ("
...
// Compare within an Epsilon to ensure layer datas are similar within reason
template <typename T, typename T_EP>
bool LayerData::compareWithin(const LayerData &other,
                              const T_EP epsilon) const {
  return epsilon > compare<T>(other);
}

template <typename T, typename T_EP>
bool LayerData::compareWithinPrint(const LayerData &other,
                                   const T_EP epsilon) const {
  return compareArrayWithinPrint(getData<T>(), other.getData<T>(), params.dims,
                                 epsilon);
}

// Allocate the layer output buffer
template <typename T> void Layer::allocateOutputBuffer() {
  outData.allocData<T>();
}
