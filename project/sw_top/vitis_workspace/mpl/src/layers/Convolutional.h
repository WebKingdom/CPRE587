...
public:
  ConvolutionalLayer(const LayerParams inParams, const LayerParams outParams,
                     ...
                     const fp32 maxW, const fp32 maxQ, const int qbits)
      : Layer(inParams, outParams, LayerType::CONVOLUTIONAL),
        weightParam(weightParams), weightData(weightParams),
        biasParam(biasParams), biasData(biasParams), maxW(maxW), maxQ(maxQ), qbits(qbits) {}
  ...
  const fp32 getMaxW() const {return maxW; }
  const fp32 getMaxQ() const {return maxQ; }
  const int getBits() const {return qbits; }
